...
const getModule = asyncHandler(async (req, res, next) => {
  ...
  if (!modules || modules.length === 0) {
    return next(
      new ErrorResponse(`Module not found with id ${req.params.id}`, 404),
    );
  }
...
const createModule = asyncHandler(async (req, res, next) => {
  ...
  if (existingModule) {
    return next(
      new ErrorResponse(
        `Module with order ${order} already exists for this course`,
        400,
      ),
    );
  }
...
const deleteModule = asyncHandler(async (req, res, next) => {
  ...
  const contentCountResult = await mongoose.connection
    .collection("contents")
    .aggregate(contentCountPipeline)
    .toArray();
  const contentCount =
    contentCountResult.length > 0 ? contentCountResult[0].total : 0;
  ...
  if (contentCount > 0) {
    return next(
      new ErrorResponse(
        `Cannot delete module with ${contentCount} associated content items. Delete content first.`,
        400,
      ),
    );
  }
  ...
  const statsPipeline = [
    ...
    },
    {
      $project: {
        ...
        publishedModules: 1,
        unpublishedModules: {
          $subtract: ["$totalModules", "$publishedModules"],
        },
...
const getModuleContentStructure = asyncHandler(async (req, res, next) => {
  ...
  const contentStructure = await mongoose.connection
    .collection("contents")
    .aggregate(pipeline)
    .toArray();
