...
const validateCourseInput = (req, res, next) => {
  ...
  if (schedule_pattern) {
    if (
      schedule_pattern.frequency &&
      !["daily", "weekly", "biweekly", "monthly", "custom"].includes(
        schedule_pattern.frequency,
      )
    ) {
...
const getCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!courses || courses.length === 0) {
    return next(
      new ErrorResponse(
        `Course not found with id, slug, or code of ${req.params.id}`,
        404,
      ),
    );
  }
                    ...
                    branches: [
                      {
                        case: {
                          $in: [
                            "$$item.__t",
                            ["LiveClasses", "RecordedClasses", "Sessions"],
                          ],
                        },
                        then: {
                          $concat: [
